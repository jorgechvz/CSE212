...
class Translator:
    ...
    def add_word(self, from_word, to_word):
        ...
        """
        self.words[from_word] = to_word # Add the word and its translation to the dictionary
    ...
    def translate(self, from_word):
        ...
        return self.words.get(from_word, "???") # Get the translation from the dictionary, or return "???" if not found
...
def summarize_degrees(filename):
    ...
    with open(filename) as file_in:
        for line in file_in:
            ...
            # ADD YOUR CODE HERE
            fields = line.strip().split(",") # Remove leading/trailing spaces and split the line by commas
            degree = fields[3].strip()  # Degree information is in the 4th column
...
# Sample Test Cases (may not be comprehensive) 
print("\n=========== PROBLEM 2 TESTS ===========")
print(summarize_degrees('L06/census.txt')) # You might need to add a path for the file
...
def is_anagram(word1, word2):
    ...
    # Count the occurrences of each letter in word1 using a dictionary
    letter_count = {}
    for letter in word1:
        letter_count[letter] = letter_count.get(letter, 0) + 1
    
    # Compare the letter counts of word1 and word2
    for letter in word2:
        if letter in letter_count and letter_count[letter] > 0:
            letter_count[letter] -= 1
        else:
            # If a letter is not in the dictionary or its count is 0, word2 is not an anagram
            return False
    
    # If all letters in word2 are accounted for, word1 and word2 are anagrams
    return True
...
class Maze:
    ...
    def move_left(self):
        ...
        """
        new_x = self.curr_x - 1
        # Check if the new position for left is valid and accessible
        if (new_x, self.curr_y) in self.maze_map and self.maze_map[(new_x, self.curr_y)][1]:
            self.curr_x = new_x # Update x-coordinate if the move is valid
        else:
    ...
    def move_right(self):
        ...
        """        
        new_x = self.curr_x + 1
        # Check if the new position for right is valid and accessible
        if (new_x, self.curr_y) in self.maze_map and self.maze_map[(new_x, self.curr_y)][0]:
            self.curr_x = new_x
        else:
            print("Can't go that way!")
    ...
    def move_up(self):
        ...
        """
        new_y = self.curr_y - 1
        # Check if the new position for up is valid and accessible
        if (self.curr_x, new_y) in self.maze_map and self.maze_map[(self.curr_x, new_y)][3]:
            self.curr_y = new_y
        else:
            print("Can't go that way!")
    ...
    def move_down(self):
        ...
        """
        new_y = self.curr_y + 1
        # Check if the new position for down is valid and accessible
        if (self.curr_x, new_y) in self.maze_map and self.maze_map[(self.curr_x, new_y)][2]:
            self.curr_y = new_y
        else:
            print("Can't go that way!")
    ...
    def show_status(self):
        ...
        print(f"Current location ({self.curr_x} , {self.curr_y})")
...
def earthquake_daily_summary():
    ...
    # Iterate through each earthquake in the response data
    for earthquake in data['features']:
        # Extract earthquake location and magnitude from the properties of each earthquake
        place = earthquake['properties']['place'] # Extract location
        magnitude = earthquake['properties']['mag'] # Extract magnitude
        # Print the location and magnitude of the current earthquake
        print(f"{place} - Mag {magnitude:.2f}")
